...
namespace CASCEdit.ZLib
{
    public static partial class zlib
    {
        ...
        public static int deflateInit(z_stream strm, int level, int bits = MAX_WBITS)
        {
        ...
        public static int deflateSetDictionary(z_stream strm, byte[] dictionary, uint dictLength)
        {
            ...
            Buffer.BlockCopy(dictionary, dictionary_ind, s.window, 0, (int)length);
        ...
        static void flush_pending(z_stream strm)
        {
            ...
            Buffer.BlockCopy(s.pending_buf, s.pending_out, strm.out_buf, strm.next_out, (int)len);
        ...
        static int read_buf(z_stream strm, byte[] buf, int buf_ind, uint size)
        {
            ...
            Buffer.BlockCopy(strm.in_buf, (int)strm.next_in, buf, buf_ind, (int)len);
            strm.next_in += len;
            strm.total_in += len;
        ...
        static void fill_window(deflate_state s)
        {
            ...
            do
            {
                ...
                if (s.strstart >= wsize + s.w_size - MIN_LOOKAHEAD)
                {
                    ...
                    Buffer.BlockCopy(s.window, (int)wsize, s.window, 0, (int)wsize);
